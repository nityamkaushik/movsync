...
function renderParticipants(container, users, isHost) {
  ...
  for (const user of users) {
    ...
    info.innerHTML = `
      ...
      <span class="participant-status ${user.isHost ? 'status-host' : 'status-waiting'}">
        ${user.isHost ? 'Host' : 'Waiting'}
      </span>
    `;
  ...
  if (isHost) {
    const startBtn = container.querySelector('#startWatchingBtn');
    const otherCount = users.filter((user) => !user.isHost).length;
    if (startBtn) startBtn.disabled = otherCount < 1;
  }
}
